...
class Event:
    ...
    def __init__(
        ...
        default_opening_hours: Tuple[int, int] = (
            20, 35),  # (inclusive, non-inclusive), (10:00 AM, 5:30 PM)
        day_resolution: int = 48,  # 48 (30-minute slots)
    ):
        """
    ...
    def __str__(self) -> str:
        ...
        unit = 24*60 // 48
        hours_str = "\n".join(
            f"  {day.name}: {f'{times[0]//2:02d}:{(times[0] % 2)*unit:02d}-{(times[1])//2:02d}:{((times[1]) % 2)*unit:02d}' if times is not None else 'Closed'}"
        ...
        if (
            ...
            bonus_time = f"\nBonus Time: {self.bonus_start//2:02d}:{(self.bonus_start % 2)*unit:02d}-{self.bonus_end//2:02d}:{(self.bonus_end % 2)*unit:02d}"
            bonus_time += f"\nBonus EXP: {self.bonus_exp}"

        return (
            f"Event: {self.name} (ID: {self.id})\n"
            f"Duration: {self.duration//2}h{(self.duration % 2)*unit}m\n"
            f"Cost: ${self.cost:.2f}\n"
...
class Itinerary:
    def __init__(self, start_day: Day, num_days: int, budget: float,
                 ...
                 min_travel_time_between_events: int = MINIMUM_TRAVEL_TIME_BETWEEN_EVENTS,
                 min_travel_cost_between_events: float = MINIMUM_TRAVEL_COST_BETWEEN_EVENTS):
        ...
        self.min_travel_time_between_events = min_travel_time_between_events
        self.min_travel_cost_between_events = min_travel_cost_between_events
        self.itinerary_start_event = Event(
            ...
            duration=0,
            base_exp=0.0,
            opening_hours=[(0, day_resolution) for day in Day],
        )
        self.total_cost = 0.0
    ...
    def schedule_event(
        ...
        Returns: 0 if successful; other non-zero code if failed. The code is as follows:
        ...
        prev_to_current_event_travel_cost = self.travel_cost_mat.get(
            (prev_event.id, event.id), self.min_travel_cost_between_events)

        # We should only consider travel cost to next event if we're actually replacing a path
        # between prev_event and next_event with a path that goes through our new event
        if next_event.id != self.itinerary_start_event.id or prev_event.id != self.itinerary_start_event.id:
            # Calculate original direct travel cost between prev and next event (which we're replacing)
            prev_to_next_direct_travel_cost = self.travel_cost_mat.get(
                (prev_event.id, next_event.id), self.min_travel_cost_between_events)

            # Calculate new path cost through our current event
            current_to_next_event_travel_cost = self.travel_cost_mat.get(
                (event.id, next_event.id), self.min_travel_cost_between_events)

            # The additional cost is the difference between the new path and the direct path
            ...
            # If there's no next event or prev event, we're just adding travel to/from this event
            current_to_next_event_travel_cost = self.travel_cost_mat.get(
                (event.id, next_event.id), self.min_travel_cost_between_events)
            additional_travel_cost = prev_to_current_event_travel_cost + \
                current_to_next_event_travel_cost
        ...
        if self.total_cost + additional_travel_cost + event.cost > self.budget:
            logger.info(
                ...
                + f"but the itinerary cost ({self.total_cost}) + additional travel cost ({additional_travel_cost}) + event cost ({event.cost})"
                + f" exceeds the budget ({self.budget})"
            )
            return 5
        prev_to_current_event_travel_time = self.travel_time_mat.get(
            (prev_event.id, event.id), self.min_travel_time_between_events)
        current_to_next_event_travel_time = self.travel_time_mat.get(
            (event.id, next_event.id), self.min_travel_time_between_events)
        if prev_slot != -1 and \
                prev_slot + prev_to_current_event_travel_time >= start_slot:
            logger.info(
                f"Attempted to schedule event {event.name} (ID: {event.id}) after {prev_event.name} (ID: {prev_event.id}),"
            ...
            return 6
        if next_slot != -1 and \
                (end_slot - 1) + current_to_next_event_travel_time >= next_slot:
            logger.info(
                f"Attempted to schedule event {event.name} (ID: {event.id}) after {prev_event.name} (ID: {prev_event.id}),"
                + f"but the end slot of the current event ({end_slot}) + travel time ({current_to_next_event_travel_time})"
                + f" is greater than or equal to next event start slot ({next_slot})"
            )
            return 6
        if next_event.id != self.itinerary_start_event.id or prev_event.id != self.itinerary_start_event.id:
            prev_to_next_direct_travel_cost = self.travel_time_mat.get(
                (prev_event.id, next_event.id), self.min_travel_time_between_events)
            self.total_gap_time += ((start_slot - prev_slot - prev_to_current_event_travel_time) +
                                    (next_slot - end_slot - current_to_next_event_travel_time) -
                                    (next_slot - prev_slot - prev_to_next_direct_travel_cost))
        else:
            current_to_next_event_travel_cost = self.travel_time_mat.get(
                (event.id, next_event.id), self.min_travel_time_between_events)
        ...
        self.scheduled_events[event.id] = (event, day, start_slot, end_slot)
        logger.info(
    ...
    def calculate_day_cost(self, day_index: int) -> Tuple[float, float]:
        ...
        # Extract the unique events in order they appear that day
        events_in_order = []
        prev_event = None
        ...
        if events_in_order:
            day_travel_cost += self.travel_cost_mat.get(
                (events_in_order[-1].id, self.itinerary_start_event.id),
                self.min_travel_cost_between_events)
    ...
    def calculate_day_travel_and_gap_time(self, day_index: int) -> Tuple[int, int]:
        ...
        for event in events_in_order:
            ...
            if prev_event != self.itinerary_start_event and event != self.itinerary_start_event:
        ...
        if events_in_order:
            day_travel_time += self.travel_time_mat.get(
                (events_in_order[-1].id, self.itinerary_start_event.id),
                self.min_travel_time_between_events)
...
def score_day_itinerary(itinerary: Itinerary, day_index: int, w_xp: float, w_count: float, w_cost: float, w_dur: float, w_travel_time: float, w_gap: float) -> float:
    ...
    score += (-w_travel_time * day_travel_cost)
    return score
